...
rhel_default_install() {
    ...
    case "$PM" in
        dnf) dnf_install $RHEL_BASE; dnf_install ${RHEL_PYTHON-python3 python3-pip python3-devel python3-numpy} ;;
        yum) yum_install $RHEL_BASE; yum_install ${RHEL_PYTHON-python3 python3-pip python3-devel python3-numpy} ;;
    esac
}
...
el8_default_install() {
    ...
    # bare python3 here; 3.11 is what el8 actually uses (numpy comes from the
    # test venv via uv pip, so no system numpy package is needed on el8).
    local RHEL_PYTHON=""
    rhel_default_install
    dnf_install \
