...
int RedisModule_OnLoad(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    if (argc % 2) {
        BAIL("Invalid number of arguments passed", NULL); // LCOV_EXCL_LINE
    }
    ...
    for (int ii = 0; ii < argc; ii += 2) {
        if (!rsStrcasecmp(argv[ii], "initial_size")) {
            ...
            if (RedisModule_StringToLongLong(argv[ii + 1], &v) == REDISMODULE_ERR) {
                BAIL("Invalid argument for 'INITIAL_SIZE'", NULL); // LCOV_EXCL_LINE
            }
            if (v > 0) {
                BFDefaultInitCapacity = v;
            } else {
                BAIL("INITIAL_SIZE must be > 0", NULL); // LCOV_EXCL_LINE
            }
        } else if (!rsStrcasecmp(argv[ii], "error_rate")) {
            double d;
            if (RedisModule_StringToDouble(argv[ii + 1], &d) == REDISMODULE_ERR) {
                BAIL("Invalid argument for 'ERROR_RATE'", NULL); // LCOV_EXCL_LINE
            } else if (d <= 0) {
                BAIL("ERROR_RATE must be > 0", NULL); // LCOV_EXCL_LINE
            } else {
            ...
            if (RedisModule_StringToLongLong(argv[ii + 1], &l) == REDISMODULE_ERR || l == 0) {
                BAIL("Invalid argument for 'CF_MAX_EXPANSIONS'", NULL); // LCOV_EXCL_LINE
            }
            CFMaxExpansions = l;
        } else {
            BAIL("Unrecognized option", NULL); // LCOV_EXCL_LINE
        } 
    } 
