...
static int CreateCMSKey(RedisModuleCtx *ctx, RedisModuleString *keyName, long long width, long long depth,
						CMSketch **cms, RedisModuleKey **key) {
	if (*key == NULL) {
...
static int parseCreateArgs(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, long long *width,
						   long long *depth) {
	if (argc == 4) {
		...
		if ((RedisModule_StringToDouble(argv[3], &overEst) != REDISMODULE_OK) || overEst <= 0 || overEst >= 1) {
			INNER_ERROR("CMS: invalid overestimation value");
		}
		if ((RedisModule_StringToDouble(argv[4], &prob) != REDISMODULE_OK) || (prob <= 0 || prob >= 1)) {
...
int CMSketch_Create(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
	...
	if ((argc == 4 && !RMUtil_ArgIndex("cms.initbydim", argv, argc)) == 0 &&
		(argc == 5 && !RMUtil_ArgIndex("cms.initbyprob", argv, argc)) == 0) {
...
/*          After change of API                  */
/*************************************************/
static int parseCreateArgsNew(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, long long *width,
							  long long *depth) {
	long long size = 0;
	double overEst = 0, prob = 0;
	...
	if ((RedisModule_StringToDouble(argv[4], &overEst) != REDISMODULE_OK) || overEst <= 0 || overEst >= 1) {
		INNER_ERROR("CMS: invalid over estimation value");
	}
	if ((RedisModule_StringToDouble(argv[5], &prob) != REDISMODULE_OK) || (prob <= 0 || prob >= 1)) {
		INNER_ERROR("CMS: invalid prob value");
	}
...
static int parseIncrByArgs(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, CMSPair **pairs, int qty) {
	for (int i = 0; i < qty; ++i) {
...
static int parseMergeArgs(RedisModuleCtx *ctx, RedisModuleString **argv, int argc, CMSketch **dest, CMSketch **cmsArray,
	...
	for (int i = 0; i < numKeys; ++i) {
		if (pos == -1) {
			weights[i] = 1;
		} else if (RedisModule_StringToLongLong(argv[3 + numKeys + 1 + i], &weights[i]) != REDISMODULE_OK) {
...
void CMSRdbSave(RedisModuleIO *io, void *obj) {
	...
	RedisModule_SaveStringBuffer(io, (const char *)cms->array, cms->width * cms->depth * sizeof(uint32_t));
}
